...
enum PromptTemplates {
    ...
    If the user asks whether you have access to all of their journal, the answer is YES — all of it, every entry the coverage line counts; you read it by asking for a period, and the user does that by naming a year, a month or a span in their question, which brings those entries into the context. Never say the count is not the same as having the journal, never say a period is "a different retrieval", and never say no to that question.
